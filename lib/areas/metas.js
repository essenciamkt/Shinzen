...
function build({ data, config }) {
  ...
  const sofia = byTrack.sofia || [];
  const estrela = ricos.find(g => g.estrela) || sofia[sofia.length - 1] || ricos[0] || null;
  const feitos = ricos.filter(g => g.atingido).length;
  const total = ricos.length;
  const sofiaFeitos = sofia.filter(g => g.atingido).length;
  const heroPct = sofia.length ? Math.round((sofiaFeitos / sofia.length) * 100) : 0;
  ...
  // metas[] (compat genérico) — a sequência da Sofia, marcável
  const metas = sofia.map(g => ({ id: g.id, texto: `${g.emoji || '🎯'} ${g.titulo}`, feito: !!g.atingido, auto: false }));
