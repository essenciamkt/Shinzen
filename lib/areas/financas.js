...
function build({ data }) {
  ...
  // ── Placas de meta (faturamento acumulado) ──
  const PLACAS = [
    { valor: 1e3,   nome: 'Primeiro Mil',   tier: 'bronze' },
    { valor: 3e3,   nome: 'Três Mil',       tier: 'prata' },
    { valor: 5e3,   nome: 'Cinco Mil',      tier: 'ouro' },
    { valor: 10e3,  nome: 'Dez Mil',        tier: 'platina' },
    { valor: 20e3,  nome: 'Vinte Mil',      tier: 'esmeralda' },
    { valor: 50e3,  nome: 'Cinquenta Mil',  tier: 'safira' },
    { valor: 100e3, nome: 'Cem Mil',        tier: 'rubi' },
    { valor: 200e3, nome: 'Duzentos Mil',   tier: 'diamante' },
    { valor: 500e3, nome: 'Meio Milhão',    tier: 'obsidiana' },
    { valor: 1e6,   nome: 'Um Milhão',      tier: 'lendaria' },
  ];
  // faturamento total: campo dedicado, ou soma do histórico de receita mensal.
  const somaHist = Array.isArray(historico.receitaMes)
    ? historico.receitaMes.reduce((s, p) => s + (Number(p.v) || 0), 0) : 0;
  const faturamentoTotal = (typeof campos.faturamentoTotal === 'number' && campos.faturamentoTotal > 0)
    ? campos.faturamentoTotal : somaHist;
  const placas = PLACAS.map((p, i) => ({ ...p, idx: i, conquistada: faturamentoTotal >= p.valor }));
  const proximaPlaca = placas.find(p => !p.conquistada) || null;
  const aliquotaImposto = (typeof campos.aliquotaImposto === 'number' && campos.aliquotaImposto >= 0)
    ? campos.aliquotaImposto : 6;

  // Ponte com Trabalho: a-receber = soma dos clientes fechados/entregando não pagos.
  const clientes = (data && data.areas && Array.isArray(data.areas.trabalho?.clientes)) ? data.areas.trabalho.clientes : [];
  const aReceberAuto = clientes
    .filter(c => c.estagio === 'fechado' || c.estagio === 'entregando')
    .reduce((s, c) => s + (Number(c.valor) || 0), 0);

  const lucro = (typeof campos.receitaMes === 'number' && typeof campos.despesasMes === 'number')
    ? campos.receitaMes - campos.despesasMes
    : null;
  const margem = (lucro != null && campos.receitaMes > 0)
    ? Math.round((lucro / campos.receitaMes) * 100)
    : null;
  const runway = (typeof campos.reservaEmergencia === 'number' && campos.despesasMes > 0)
    ? campos.reservaEmergencia / campos.despesasMes
    : null;
  ...
  const resumo = [
    ...
    },
    {
      ...
      delta: margem != null ? `margem ${margem}%` : null,
    },
    {
      label: 'Runway',
      valor: runway != null ? `${runway.toLocaleString('pt-BR',{maximumFractionDigits:1})} m` : '—',
      delta: 'fôlego',
    },
  ];
  ...
  return {
    ...
    detalhe: {
      ...
      camposEditaveis: [
        ...
        { id: 'faturamentoTotal',  label: 'Faturamento total',  tipo: 'number', unidade: 'R$',  historico: true },
        { id: 'receitaRecorrente', label: 'Receita recorrente', tipo: 'number', unidade: 'R$',  historico: false },
        { id: 'metaReceita',       label: 'Meta de receita',    tipo: 'number', unidade: 'R$',  historico: false },
        { id: 'aReceber',          label: 'A receber',          tipo: 'number', unidade: 'R$',  historico: false },
