...
const PROGRESS_DEFS = {
  ...
  rotina: {
    badges: [
      { id: 'exec-7',      nome: 'Executor',      icone: '🎯', desc: '7 dias batendo a meta de execução.',
        check: c => histCount(c.historico, 'produtividadeH', v => v >= (c.campos.metaExecucao || 3)) >= 7, meta: 7,
        atualFn: c => histCount(c.historico, 'produtividadeH', v => v >= (c.campos.metaExecucao || 3)) },
      { id: 'foco-mes',    nome: 'Foco de Aço',   icone: '🔥', desc: '30 dias com bloco de execução.',
        check: c => histCount(c.historico, 'produtividadeH', v => v >= 1) >= 30, meta: 30,
        atualFn: c => histCount(c.historico, 'produtividadeH', v => v >= 1) },
      ...
      { id: 'sistema-ok',  nome: 'Sistema Rodando', icone: '⚙️', desc: 'Rodou o sistema completo hoje.',
        check: c => ['sysAcordar','sysOrar','sysPlano','sysBloco','sysDormir'].every(k => Number(c.campos[k]) >= 1) },
      { id: 'manha-7',     nome: 'Dono da Manhã', icone: '☀️', desc: '7 manhãs vencidas.',
        check: c => Math.min(histCount(c.historico,'sysAcordar',v=>v>=1), histCount(c.historico,'sysOrar',v=>v>=1), histCount(c.historico,'sysPlano',v=>v>=1)) >= 7,
        meta: 7, atualFn: c => Math.min(histCount(c.historico,'sysAcordar',v=>v>=1), histCount(c.historico,'sysOrar',v=>v>=1), histCount(c.historico,'sysPlano',v=>v>=1)) },
      { id: 'streak-14',   nome: 'Rotina de Aço', icone: '⛓️', desc: '14 dias seguidos de check-in.',
        check: c => c.streak.atual >= 14, meta: 14, atualFn: c => c.streak.atual },
    ],
    missoes: [
      { id: 'bater-execucao', nome: 'Bater meta de execução', icone: '🎯', periodo: 'dia', xp: 3,
        check: c => typeof c.campos.produtividadeH === 'number' && c.campos.produtividadeH >= (c.campos.metaExecucao || 3) },
      { id: 'sistema-completo', nome: 'Rodar o sistema completo', icone: '⚙️', periodo: 'dia', xp: 2,
        check: c => ['sysAcordar','sysOrar','sysPlano','sysBloco','sysDormir'].every(k => Number(c.campos[k]) >= 1) },
      { id: 'acordar-cedo', nome: 'Acordar cedo',      icone: '🌅', periodo: 'dia', xp: 1,
        check: c => typeof c.campos.acordouCedo === 'number' ? c.campos.acordouCedo <= 7 : !!c.campos.acordouCedo },
    ],
  },
  ...
  mente: {
    badges: [
      { id: 'calmo-7',    nome: 'Sereno',        icone: '🌿', desc: '7 dias com ansiedade baixa (≤4).',
        check: c => histCount(c.historico, 'ansiedade', v => v >= 1 && v <= 4) >= 7, meta: 7,
        atualFn: c => histCount(c.historico, 'ansiedade', v => v >= 1 && v <= 4) },
      { id: 'regulador',  nome: 'Regulador',     icone: '🌬️', desc: '7 vezes que pausou e respirou.',
        check: c => histCount(c.historico, 'fezReset', v => v >= 1) >= 7, meta: 7,
        atualFn: c => histCount(c.historico, 'fezReset', v => v >= 1) },
      { id: 'humor-bom',  nome: 'Bem-estar',     icone: '😊', desc: 'Humor ≥ 8 hoje.',
        check: c => typeof c.campos.humor === 'number' && c.campos.humor >= 8 },
      { id: 'grato-mes',  nome: 'Coração Cheio', icone: '🙏', desc: '30 dias praticando gratidão.',
        check: c => histCount(c.historico, 'gratidaoHoje', v => v >= 1) >= 30, meta: 30,
        atualFn: c => histCount(c.historico, 'gratidaoHoje', v => v >= 1) },
      { id: 'mente-forte', nome: 'Mente Forte',  icone: '🧠', desc: 'Ansiedade ≤3 e humor ≥7 hoje.',
        check: c => typeof c.campos.ansiedade === 'number' && c.campos.ansiedade <= 3
          && typeof c.campos.humor === 'number' && c.campos.humor >= 7 },
    ],
    missoes: [
      { id: 'registrar-estado', nome: 'Registrar como me sinto', icone: '🧠', periodo: 'dia', xp: 1,
        check: c => typeof c.campos.ansiedade === 'number' || typeof c.campos.humor === 'number' },
      { id: 'gratidao', nome: 'Praticar gratidão',     icone: '🙏', periodo: 'dia', xp: 1,
        check: c => Number(c.campos.gratidaoHoje) >= 1 },
    ],
  },

  diario: {
    badges: [
      { id: 'diario-30', nome: 'Escritor', icone: '📓', desc: '30 dias escrevendo no diário.',
        check: c => histCount(c.historico, 'escritaHoje', v => v >= 1) >= 30, meta: 30,
        atualFn: c => histCount(c.historico, 'escritaHoje', v => v >= 1) },
    ],
    missoes: [
      { id: 'escrever', nome: 'Escrever no diário', icone: '📓', periodo: 'dia', xp: 1,
        check: c => Number(c.campos.escritaHoje) >= 1 },
    ],
  },
};
