...
function addCapitulo(areaId, ano, titulo) {
  const data = readData();
  const area = ensureArea(data, areaId);
  const anoNum = Number(ano) || new Date().getFullYear();
  const ordem = area.capitulos.filter(c => c.ano === anoNum).length;
  const c = { id: 'cap-' + Date.now(), ano: anoNum, titulo: String(titulo || '').slice(0, 80), ordem };
