...
const { readData, recordScoreSnapshot } = require('./datastore');
const { build: buildSocio } = require('./socio');
...
const { build: buildAreaDiario }     = require('./areas/diario');
...
async function build({ fresh = false } = {}) {
  ...
  const areaDiario     = buildAreaDiario(ctx);

  const DEFAULT_ORDER = ['claude-code', 'saude', 'financas', 'trabalho', 'metas', 'igreja', 'lazer', 'rotina', 'mente', 'diario'];
  ...
  // ── tendência: snapshot diário do score (upsert idempotente) + série p/ sparkline ──
  try { life.trend = recordScoreSnapshot(today, life).slice(-30); } catch { life.trend = []; }
  ...
  const meta = {
    ...
    mrrAlvo: (config.meta && config.meta.mrrAlvo != null) ? config.meta.mrrAlvo : 5000,
  };

  // ── Modo Sócio: voz do founder-advisor confrontando o número do dia ──
  try { life.socio = buildSocio({ areas, meta, config, today, overall: life.overall }); }
  catch { life.socio = null; }
  ...
  return {
    ...
    life,
    meta,
    theme: config.theme || 'carvao',
    changelog: Array.isArray(data.changelog) ? [...data.changelog].sort((a, b) => (a.data < b.data ? 1 : a.data > b.data ? -1 : b.criadoEm - a.criadoEm)) : [],
    areas,
    claudeRaw,
    health: {
      ok: true,
